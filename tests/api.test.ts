...
describe('health', () => {
  it('reports ok without any API key configured', async () => {
...
describe('profiles', () => {
  it('creates and lists a profile', async () => {
    ...
    const list = await request(app).get('/api/profiles').expect(200);
  ...
  it('rejects a profile missing required fields', async () => {
    await request(app).post('/api/profiles').send({ name: 'x' }).expect(400);
  });
  ...
  it('updates a profile', async () => {
    const { body } = await request(app).post('/api/profiles').send(validProfile);
  ...
  it('404s on an unknown profile', async () => {
    await request(app).patch('/api/profiles/nope').send({ radius_miles: 5 }).expect(404);
  });

  it('deletes a profile', async () => {
    const { body } = await request(app).post('/api/profiles').send(validProfile);
    await request(app).delete(`/api/profiles/${body.id}`).expect(204);
    const list = await request(app).get('/api/profiles').expect(200);
...
describe('sources', () => {
  ...
  it('rejects a duplicate URL on the same profile with a clear message', async () => {
    ...
    await request(app).post('/api/sources').send(body).expect(201);
    const res = await request(app).post('/api/sources').send({ ...body, name: 'B' }).expect(409);
  ...
  it('makes a reactivated source immediately due for scanning', async () => {
    ...
    await request(app).patch(`/api/sources/${body.id}`).send({ status: 'paused' }).expect(200);
    const res = await request(app)
  ...
  it('filters by profile', async () => {
    ...
    const res = await request(app).get(`/api/sources?profileId=${a}`).expect(200);
...
describe('work types', () => {
  ...
  it('requires a profileId to list', async () => {
    await request(app).get('/api/work-types').expect(400);
  });
  ...
  it('disambiguates a duplicate name', async () => {
    ...
    const a = await request(app).post('/api/work-types').send(body).expect(201);
    const b = await request(app).post('/api/work-types').send(body).expect(201);
...
describe('settings', () => {
  it('returns defaults and accepts a known key', async () => {
    ...
    await request(app).put('/api/settings').send({ perRunBudgetUsd: '9' }).expect(200);
  ...
  it('ignores keys that are not real settings', async () => {
    await request(app).put('/api/settings').send({ nonsense: 'x' }).expect(200);
    const res = await request(app).get('/api/settings').expect(200);
